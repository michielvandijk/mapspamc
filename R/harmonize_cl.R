...
check_cl <- function(df, adm_lvl, ac, param){
  ...
  cl_check <- df %>%
    ...
    dplyr::summarize(cropland = sum(cl, na.rm = T),
    ...
    dplyr::mutate(cl_short = cropland-pa,
                  max_cl_short = max_cropland-pa,
                  ga_short = grid_area-pa) %>%
    dplyr::select(adm_code, adm_name, adm_level, cropland, max_cropland, grid_area, pa, cl_short, max_cl_short,
                  ga_short) %>%
