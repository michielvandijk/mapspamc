...
create_adm_map_pdf <- function(param, font_size = 3) {
  ...
  if (param$adm_level %in% c(1, 2)) {
    ...
    adm1_plot <- ggplot() +
      ...
      geom_text(
        data = adm1_name, aes(x = X, y = Y, label = adm1_name), size = font_size,
        check_overlap = FALSE
      ) +
      theme(
  ...
  if (param$adm_level %in% c(2)) {
    ...
    adm2_plot <- ggplot() +
      ...
      scale_fill_manual(values = cols) +
      geom_text(
        data = adm2_name, aes(x = X, y = Y, label = adm2_name), size = font_size,
        check_overlap = FALSE
      ) +
      theme(
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
  ...
  pdf(
    file = file.path(temp_path, glue::glue("adm_map_{param$res}_{param$yea}_{param$iso3c}.pdf")),
