...
To start running one of the `mapspamc` models several steps need to be taken to setup the model (note that we assume that all required software was [installed](installation.html) and is working). 


## Create model template

The first step is to create a project with RStudio, activity `mapspamc` and use the `create_model_template` function to  create a set of template folders and scripts that implement the six steps to run the `mapspamc` models. An alternative approach would be to clone the repository with one of the [country examples](country_examples.html) and use that as a starting point. We will use scripts of the Malawi example to illustrate the various steps in the vignettes. All scripts related to model setup can be found in the `01_model_setup` folder of the template. The most important is that you change the location on your hard disk where the model will be stored and set the model parameters (see below), which is done in the script `01_model_setup.r` in the template. As the model parameters are needed as input in nearly all functions, this script is always loaded at the beginning of all the other scripts using the `source()` command. In this way, you only have to setup up the model once. It also ensures that all the necessary packages are loaded into R. 


```{r eval=FALSE}
# Create a new RStudio project called mapspamc_mwi in the folder c:/temp (or an alternative location)
...
Nearly all functions in `mapspamc` need input on (1) key parameters that determine the design of the model and how it will be solved and (2) the location of raw data, which will be further processed. Both pieces of information are bundled in a `mapspamc_par` object that is created by the function `set_mapspamc_par()`. Another piece of information that is also stored in the `mapspamc_par` object is the coordinate reference system of the maps. This is automatically set to WSG84 (epsg:4326).
...
- **Location of mapspam_db (`db_path`).** Optionally the user can set a folder for the location of the mapspamc database. This might be convenient if you want to use a server to store the database (> 5 GB).  A folder with the name `mapspamc_db` will be automatically created in the `db_path` set by the user. If `db_path` is not specified the `mapspamc_db` folder will be created in the `model_db` folder.

- **Location of GAMS (`gams_path`).** Optionally the user can set a folder for the location of GAMS and the libraries that are needed to process GAMS gdx with R. If GAMS is installed correctly, `mapspamc` will automatically find the GAMS system directory. However, occasionally the location of GAMS is not stored correctly by Windows, causing problems. Hence, we always recommend setting the location of GAMS manually. This might also be convenient when multiple versions of GAMS are installed on one machine and one is preferred over another. 

The example below sets the `mapspamc` parameters for Malawi and stores them in an object called `param`. Param is an arbitrary name but we recommend not to change it as we refer to `param` in the documentation of most functions.
...
`mapspamc` includes a function to create all necessary folders `create_folders()`. The only input required is `param` which contains all the model parameters. The function will create three folders: (1) `raw_data` is created in the main `spamc_path` unless indicated otherwise by the user and serves to store all the raw input data, (2) `processed_data` will be created in the main `mapspamc_path` and is used to save all the intermediary data products as well as the final results and (3) `mappings` will be created in the main `mapspamc_path` and contains all the lists and mappings that are needed to define the model (e.g. number crops) and harmonize various data sources (concordance tables). All tables are in csv format and will be created automatically into the `mappings` folder. Note that it is possible to adjust some of these tables if needed (e.g. adding a new crop). `create_folders()` will only create csv files if the files do not exist. Hence, updated files will not be overwritten. If for some reason you want to recreate the original tables, simply remove the folder and/or files and they will be created again when `create_spam_folders` is run. 
...
# load shapefile
adm_map_raw <- read_sf(file.path(param$mapspamc_path, glue("raw_data/adm/{iso3c_shp}")))
