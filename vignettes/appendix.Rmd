...
## Solving the `mapspamc` algorithms

The General Algebraic Modeling System (GAMS) [@GAMSDevelopmentCorporation2019] is used to solve both the cross-entropy and fitness score models that are available in `mapspamc`. To deal with inconsistencies, the first step is to closely inspect the data, and in particular the subnational crop statistics, for data entry errors (e.g. missing values) and, if needed, repair them. If there are still problems, we recommend the following 'hierarchy' of addressing inconsistencies and errors in the data:
...
Also similar to global SPAM, `mapspamc` distinguises between four different farming systems:
