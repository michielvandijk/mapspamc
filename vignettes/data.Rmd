...
The synergy cropland approach combines all available (global) cropland maps and creates a ranking for each grid cell that measures the level of agreement between the products. In the `mapspamc` data-preparation stage, the grid cells with the least uncertainty are selected first when allocating physical area of individual crops. Apart from the ranking, the synergy cropland approach also creates maps with the medium and maximum cropland area per grid cell. The medium area product is used as the base layer by `mapspamc` but in case this is not sufficient to allocate all the physical area, grid cells from the maximum cropland map can be used (see [Combine input data](combine_input_data.html) for more information on how this is done).  
