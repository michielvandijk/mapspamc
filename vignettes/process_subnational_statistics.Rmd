...
`create_statistics_template()` will create templates for the ha, fs and ci data files - also see the Malawi ha, fs and ci files for examples.  The number and depth of the administrative units will be determined by the model parameters stored in the `spamc_par` object as well as the `adm_list` file that is created with `create_adm_list()`. By default, the template will have columns for all 40 SPAM-C crops. 
...
```{r eval = FALSE}
write_csv(ha_df, file.path(param$spamc_path, glue("processed_data/agricultural_statistics/ha_adm_{param$year}_{param$iso3c}.csv")))
write_csv(fs_df, file.path(param$spamc_path, glue("processed_data/agricultural_statistics/fs_adm_{param$year}_{param$iso3c}.csv")))
write_csv(ci_df, file.path(param$spamc_path, glue("processed_data/agricultural_statistics/ci_adm_{param$year}_{param$iso3c}.csv")))
