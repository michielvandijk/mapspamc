...
To allocate the physical area statistics `mapspamcc` requires a cropland extent, which shows the location of cropland in a country for the target year. There are several global cropland products and often countries produce their own land cover map, which includes a cropland class. To account for the uncertainty in the location of cropland, @Fritz2011 combined several different products into one so-called synergy cropland map. A more recent product was prepared by @Lu2020 and used as input for the construction of SPAM2010.

The synergy cropland approach combines all available (global) cropland maps and creates a ranking for each grid cell that measures the level of agreement between the products. In the `mapspamc` data-preparation stage, the grid cells with the least uncertainty are selected first when allocating physical area of individual crops. Apart from the ranking, the synergy cropland approach also creates maps with the mean and maximum cropland area per grid cell. The mean area product is used as the base layer by `mapspamc` but in case this is not sufficient to allocate all the physical area, grid cells from the maximum cropland map can be used (see [Combine input data](combine_input_data.html) for more information on how this is done).  
