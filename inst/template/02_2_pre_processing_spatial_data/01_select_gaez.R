...
clip_gaez <- function(file_name, folder, method = "bilinear"){
  ...
  cat("\n", id)
  temp_path <- file.path(param$mapspamc_path, glue("processed_data/maps/{folder}/{param$res}"))
...
gaez_files_py <- list.files(file.path(param$raw_path, glue("gaez/potential_yield")),
                            pattern = glob2rx("*.tif"),
                            full.names = TRUE)
...
gaez_files_bs <- list.files(file.path(param$raw_path, glue("gaez/biophysical_suitability")),
                            pattern = glob2rx("*.tif"),
                            full.names = TRUE)
