...
# NOTE: the ESACCI input file is conditional on the year. So make sure you
...
# Warp and mask
input <- file.path(param$raw_path, glue("esacci/esacci_2015.tif"))
output <- align_raster(input, grid, adm_map, method = "bilinear")
plot(output)
writeRaster(output, file.path(temp_path, glue("cropmask_esa_{param$res}_{param$year}_{param$iso3c}.tif")),
            overwrite = TRUE)
